...
PatchedWritableCircuitRequest<PSCustomObject>
#>

function Initialize-PatchedWritableCircuitRequest {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => PatchedWritableCircuitRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if (!$Cid -and $Cid.length -gt 100) {
            throw "invalid value for 'Cid', the character length must be smaller than or equal to 100."
        }

        if (!$Cid -and $Cid.length -lt 1) {
...
Convert from JSON to PatchedWritableCircuitRequest<PSCustomObject>
...
PatchedWritableCircuitRequest<PSCustomObject>
#>
function ConvertFrom-JsonToPatchedWritableCircuitRequest {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => PatchedWritableCircuitRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in PatchedWritableCircuitRequest
